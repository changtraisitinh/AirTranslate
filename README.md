...
- 현재 세션과 접힌 이전 세션을 함께 관리하는 기록 화면
...
- 실시간 기록 일시정지/재개
- 정지 시 현재 기록 저장, 다음 시작 시 이전 세션으로 자동 보관
...
`일시정지`는 현재 세션을 유지한 채 입력만 멈춥니다. `정지`는 현재 기록을 저장하되 화면의 현재 세션은 남겨 둡니다. 다음에 새 캡처를 시작하면 남아 있던 기록이 접힌 이전 세션으로 보관되고 새 현재 세션이 열립니다.
...
7. 정지를 누르면 현재 기록이 저장되고, 다음 시작 시 이전 세션으로 접혀 보관됩니다.
...
- `TranslationSessionStore`: 캡처, 기록, 번역, 문단 정리, 세션 보관, 저장, 음성 출력을 조율합니다.
...
- `CaptionBoardView`: 현재/이전 세션, 원문/번역 스크롤 패널, 복사 피드백, 데시벨 반응형 waveform 미터, 아이콘형 상단 컨트롤을 표시합니다.
